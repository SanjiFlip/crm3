...
<div class="page-container">
	<form action="xtrolectrl/selectxtrole.do" method="post" id="myform2">
		<input type="hidden" name="pageNum" id="pageNum" value="${p.pageNum }">
	</form>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 	
	<a href="javascript:;" onclick="member_add('添加用户','xtrolectrl/goaddxtrole.do','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>添加角色</a>
	</span> <span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	<form action="xtrolectrl/deletemore.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${p.list }" var="role">
				<tr class="text-c">
					<td><input type="checkbox" value="${role.roleId}" name="ids"></td>
					<td>${role.roleId}</td>
					<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">${role.roleName}</u></td>
					
					<td>${role.roleDes}</td>
					<td>${role.upRoleName}</td>
					<td>${role.operator}</td>
					<td>
					<fmt:formatDate value="${role.lastModifyDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td class="td-manage">
					<a title="查看成员" href="javascript:;" onclick="member_find('查看成员','xtrolectrl/selectrolemem.do','${role.roleId}','','510')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe665;</i></a>
					<a title="添加成员" href="javascript:;" onclick="member_addrole('添加成员','xtrolectrl/selectnotrole.do','${role.roleId}','','510')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe610;</i></a>
					<a title="权限设置" href="javascript:;" onclick="member_role('权限设置','xtrolectrl/gorolemanage.do','${role.roleId}','','510')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe61d;</i></a>
