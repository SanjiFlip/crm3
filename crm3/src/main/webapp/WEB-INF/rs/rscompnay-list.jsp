...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 人事管理 <span class="c-gray en">&gt;</span> 公司管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<form action="rscompnayctrl/selectcompnay.do" method="post" id="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${rscompnay.datemin }" pattern="yyyy-MM-dd"/>"  onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${rscompnay.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
		<input type="text" value="${rscompnay.compnayName }" class="input-text" style="width:250px" placeholder="请输入公司名称" id="compnayName" name="compnayName">
	...
	<a href="javascript:;" onclick="member_add('添加公司','rscompnayctrl/goaddcompnay.do','','510')" class="btn btn-primary radius">
	<i class="Hui-iconfont">&#xe600;</i> 添加公司</a></span><span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	 <form action="rscompnayctrl/deletecompnayall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${p.list }" var="rscompnay">
		
			<tr class="text-c">
				<td><input type="checkbox" value="${rscompnay.compnayId }" name="ids"></td>
				<td>${rscompnay.compnayId }</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${rscompnay.compnayName }','rscompnayctrl/showcompnay.do','${rscompnay.compnayId }','360','400')">${rscompnay.compnayName }</u></td>
				<td>${rscompnay.mailbox }</td>
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'rscompnayctrl/enabledcompnay.do',
			data: "compnayId="+id+"&enabled=0",
			dataType: 'json',
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'rscompnayctrl/deletecompnay.do',
			data: "compnayId="+id,
			dataType: 'json',
			success: function(data){
